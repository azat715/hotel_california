...
from hotel_california.service_layer.service.hotel import add_room, find_rooms, check_room, create_order, \
    get_order_by_id, get_room_by_num, delete_order
...
async def add_room_endpoint(room: RoomAddForm, worker: UOW = Depends(get_room_worker)):
    res = add_room(**room.dict(), workers=worker)
    return JSONResponse(
        status_code=201,
        content={"room_num": res.number},
    )
...
async def booking_room_endpoint(num: int, arrival: date, departure: date,
    ...
    order = create_order(room, arrival, departure, workers=order_worker)
...
async def get_bookings_endpoint(num: int, room_worker: UOW = Depends(get_room_worker)):
    ...
    room = get_room_by_num(num, room_worker)
    res = []
    for i in room.orders:
        res.append(convert_order(i))
    return res
...
async def get_booking_endpoint(order_id: int, order_worker: UOW = Depends(get_order_worker)):
    """Получить информацию по брони

    (указываем номер брони, возвращаем дату заезда и дату отъезда)
    """
    order = get_order_by_id(order_id, order_worker)
    res = convert_order(order)
    return res
